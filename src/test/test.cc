...
void Frame() {
    ...
    if (ImGui::Begin("Test interaction")) {
        ed::SetCurrentEditor(g_Context);

        // Start interaction with editor.
        ed::Begin("My Editor2", ImVec2(0.0, 0.0f));
        ...
        if (ed::BeginCreate()) {
            ...
            if (ed::QueryNewLink(&inputPinId, &outputPinId)) {
                ...
                // TODO if only one is valid allow user to create a new node for input
                // Also attach to a valid input / output by default
            }
        }
